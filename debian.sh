...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome NamystoreTunneling ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green} Namydev® ${NC}${YELLOW}(${NC} ${green} NamyTunnel ${NC}${YELLOW})${NC}"
echo -e " © Recode By My NamystoreTunneling${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Namydevx/Register/main/ipuser | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/Namydevx/Register/main/ipuser | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/Namydevx/Register/main/ipuser | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/Namydevx/Namystore-Tunnel/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/Namydevx/Namystore-Tunnel/main/ipuser | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/Namydevx/Namystore-Tunnel/main/ipuser | grep $ipsaya | awk '{print $3}')
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ🐳","url":"https://t.me/Namydevx "},{"text":"ɪɴꜱᴛᴀʟʟ🐬","url":"https://t.me/mb2vpn"}]]}'
...
wget raw.githubusercontent.com/Namydevx/Namystore-Tunnel/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
wget -O /usr/sbin/namystore "${REPO}limit/namystore" >/dev/null 2>&1
